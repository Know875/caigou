...
-- 注意：这通常不应该发生，因为豪应该已经报价了。如果确实没有，需要手动处理
...
SELECT 
    CONCAT('cmi', SUBSTRING(MD5(CONCAT(@rfq_id, @supplier_from_id, NOW(), 'other')), 1, 25)),
    @rfq_id,
    @quote_from_id,
    @supplier_from_id,
    @final_price_from,
    '手动调整：UR神光棒转移后，为其他商品重新创建Award',
    'ACTIVE',
    NOW(),
    NOW(),
    NOW()
WHERE @other_item_id IS NOT NULL
...
SELECT 
    CASE 
        WHEN @other_item_id IS NOT NULL AND @final_price_from > 0 THEN 
            CONCAT('已为赛罗的其他商品创建新的 Award，价格: ', @final_price_from)
        ELSE '赛罗没有其他商品，无需创建新 Award'
    END AS message;
...
-- 7. 如果豪还没有 Award，且豪有 Quote，为豪创建新的 Award
-- 计算豪的 Quote 的总价（所有商品）
SET @final_price = (
    SELECT COALESCE(SUM(qi.price * ri.quantity), 0)
    FROM quote_items qi
...
SELECT 
    CASE 
        WHEN @award_to_id IS NOT NULL THEN CONCAT('豪的 Award ID: ', @award_to_id)
