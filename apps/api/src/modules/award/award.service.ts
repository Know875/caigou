...
export class AwardService {
  ...
  async findBySupplier(supplierId: string, requestOrigin?: string) {
    ...
    for (const [rfqItemId, items] of quoteItemMap) {
      ...
      // 优先查找 Award 记录，确定中标供应商（支持手动选商）
      // ⚠️ 重要：不能简单地遍历所有 Award 记录，因为一个 RFQ 可能有多个 Award（每个供应商一个）
      // 需要找到真正中标该商品的供应商，而不是第一个找到的供应商
      ...
      if (rfqId) {
        ...
        // 如果通过 Award 记录没找到，说明可能是自动选商（没有 Award 记录）
        // 这种情况下，使用价格最低的报价项
        if (!bestQuoteItem && allAwards.length === 0) {
          if (process.env.NODE_ENV === 'development') {
            this.logger.debug(`findBySupplier: 未找到 Award 记录，可能是自动选商，将使用最低报价`);
          }
        }
      }
      ...
      if (!bestQuoteItem) {
        ...
        if (instantPrice) {
          ...
          if (instantPriceQuotes.length > 0) {
            bestQuoteItem = instantPriceQuotes[0];
            if (process.env.NODE_ENV === 'development') {
              this.logger.debug(`findBySupplier: 未找到 Award 记录，使用满足一口价且最早提交的报价: ${bestQuoteItem.quote.supplierId}, 价格: ¥${bestQuoteItem.price}`);
            }
          }
        }

        // 如果没有满足一口价的，使用价格最低的报价（价格相同时，按提交时间排序）
        if (!bestQuoteItem) {
  ...
  async uploadTrackingNumber(
    ...
    if (awardId.startsWith('virtual-')) {
      ...
      // ⚠️ 重要：验证该供应商是否真的中标了该商品
      // 使用与 findBySupplier 相同的逻辑：通过 Award 记录来验证，而不是简单地比较价格
      ...
      for (const quoteItem of allQuotesForItem) {
        const matchingAward = allAwards.find(award => {
          ...
          return award.quote.items.some((qi: any) => qi.id === quoteItem.id);
        });
      ...
      // 但需要考虑一口价的情况：如果有一口价，优先选择满足一口价且最早提交的
      ...
      if (!bestQuoteItem) {
        throw new BadRequestException('No quote found for this item');
      }
      ...
      if (!bestQuoteItem) {
        ...
        if (instantPrice) {
          ...
          if (instantPriceQuotes.length > 0) {
            bestQuoteItem = instantPriceQuotes[0];
            this.logger.debug(`未找到 Award 记录，使用满足一口价且最早提交的报价: ${bestQuoteItem.quote.supplierId}, 价格: ¥${bestQuoteItem.price}`);
          }
        }
